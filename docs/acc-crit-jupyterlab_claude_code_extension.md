...
- [x] **Detection** - `sessions.bg_agents()` maps conversation id -> short agent id for every live background session, from `claude agents --json`; interactive sessions in that listing are ignored
  ...
  - log: 2026-08-01 liveness added (DEF-14): that listing is derived from the on-disk job records, not from the daemon, so it keeps listing a job whose worker exited (`state: "blocked"`) and carries stale pids elsewhere - membership alone marked 3 rows of which 2 resumed with no refusal, and the false chip made the click `claude attach`, resurrecting a dead agent. An entry now counts only when it reports a worker `pid` that still exists (`_pid_alive`); live re-check marked 3 rows -> 1
